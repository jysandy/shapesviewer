...
float4 main(PixelShaderInput input) : SV_TARGET
{
	...
	ComputedColor pointColor;
	pointColor = computePointColor(normal, toEye, vertex);
	ComputedColor spotColor;
	spotColor = computeSpotColor(normal, toEye, vertex);
	
	float4 ambient;
	ambient = directionalColor.ambient + pointColor.ambient + spotColor.ambient;

	float4 diffuse;
	diffuse = directionalColor.diffuse + pointColor.diffuse + spotColor.diffuse;

	float4 specular;
	specular = directionalColor.specular + pointColor.specular + spotColor.specular;
